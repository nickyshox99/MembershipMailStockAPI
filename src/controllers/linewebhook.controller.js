...
const lineReply = require('./../modules/lineReply');
...
module.exports = function (wsConnections) {
  ...
  const webhook = async (req, res) => {
    try {
      ...
      // เพิ่ม req.body เอาไว้เช็ค 
      try {
        MainModel.insert("line_webhook_events_raw", {
          body_json: JSON.stringify(req.body),
        });
      } catch (e) {
        console.log("Cannot insert webhook raw:", e?.message || e);
      }
      ...
      let profileData = [];
      if (sourceUserId != '') {
        let checkProfile = await MainModel.query(`SELECT * FROM line_contact WHERE bot_user_id='${botUserId}' AND user_id='${sourceUserId}'`);
